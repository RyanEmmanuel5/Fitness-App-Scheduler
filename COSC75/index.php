...
<body>
    <div class="hero">
        <div class="form-box">
            ...
            <form id="register" class="input-group" method="POST" onsubmit="return validatePassword(event)">
                <input type="text" class="input-field" placeholder="Username" name="username" id="register-username" required>
                ...
                <input type="password" class="input-field" placeholder="Re-type Password" name="repassword" id="repassword" required>
    ...
    <script>
        ...
        function validatePassword(event) {
            ...
            // Then check if the username is available via AJAX
        ...
        function checkUsernameAvailability(username, password, repassword) {
            ...
            xhr.open("POST", "register.php", true); // Send the request to register.php
        ...
        function submitRegistration(username, password, repassword) {
            var email = document.getElementById('register-email').value; // Get the email field value
            ...
            var formData = "username=" + encodeURIComponent(username) +
                           ...
                           "&email=" + encodeURIComponent(email); // Add email to the form data
            ...
            xhr.onreadystatechange = function() {
                if (xhr.readyState === 4 && xhr.status === 200) {
                    ...
                    if (response === "success") {
                        ...
                        showError(response); // If there's any error, show the error message
                    }
                }
            };

            xhr.send(formData); // Send the form data via POST
        }
        ...
        document.getElementById('login').addEventListener('submit', function(event) {
            ...
            xhr.onreadystatechange = function() {
                if (xhr.readyState === 4 && xhr.status === 200) {
                    ...
                    if (response === "success") {
                        window.location.href = "landing.php"; // Redirect to landing page
                    } else {
